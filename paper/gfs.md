...
​	**常态化的组件故障**：组件故障在 GFS 中很常见，因为文件系统由数百甚至数千台存储计算机组成，这些存储计算机由廉价的商品部件构建，并且可供相当数量的客户端计算机访问。组件的数量和质量决定了某些组件在任何给定时间都无法正常工作，有些组件无法从当前的故障中恢复过来。如应用程序错误、操作系统错误、人为错误以及磁盘、内存、连接器、网络和电源故障引起的问题。通过**持续监控**、**错误检测**、**容错**和**自动恢复**以保障系统可靠性和可用性。
...
​	GFS 提供了用户熟悉的文件系统接口，但是没有实现诸如POSIX之类的标准API。GFS 中的文件按目录层次结构组织，并由路径名标识。GFS 支持创建、删除、打开、关闭、读取和写入文件的常规操作。
​	GFS 还具有**快照**和**记录追加**操作，这是传统文件系统中都没有的。Snapshot 以低成本创建文件或目录树的副本，这对于在不影响原始文件的情况下创建备份或测试更改非常有用。Record append 允许多个客户端同时向同一个文件追加数据，同时保证每个客户端追加的原子性。这对于实现多路合并结果和生产者-消费者队列很有用，许多客户端可以同时追加到这些队列而无需加锁。这些类型的文件对于构建大型分布式应用程序非常有用，这是 GFS 的主要用途。快照和记录追加分别在第 3.4 节和第 3.3 节中进行了进一步讨论，这两节提供了有关这些功能的更多详细信息。
...
​	Client 和 ChunkServer 都不会缓存文件数据。客户端缓存几乎没有什么好处，因为大多数应用程序都会流式传输大文件或工作集太大而无法缓存。 没有客户端缓存可以消除缓存一致性问题，从而简化客户端和整个系统。但客户端会缓存元数据。 ChunkServers 不需要缓存文件数据，因为块存储为本地文件，而且 Linux 的缓冲区缓存已经将经常访问的数据保存在内存中。

​	

## 单主架构

​	GFS 只有一个 Master，它简化了设计，使 Master 能够利用全局信息做出复杂的**块放置**和**块复制**决策。 单主架构可以更好地协调和管理分布式文件系统。 Master 负责维护文件命名空间并跟踪每个数据块的位置。还负责将区块分配给区块服务器，并确保每个区块都有足够的副本以实现容错。 但是，必须尽量减少主节点参与读取和写入的情况，以防止其成为瓶颈。 客户端从不通过 Master 读取和写入文件数据，以避免请求过载。 取而代之的是，客户端会询问 Master 他们应该联系哪些 ChunkServer 来读取或写入数据。 Client 会限时缓存元数据，并直接与 ChunkServer 交互。 此架构允许高效、可扩展的数据访问，同时仍能保持容错和一致性。

![image-20230812230839569](./images/gfs/image-20230812230839569.png)

​	通过图1来介绍简单的读流程。第一步是让 Client 通过固定的块大小将应用指定的文件名和字节偏移量转换为文件内的块索引。 确定区块索引后，客户端会向 Master 发送包含文件名和块索引的请求。 然后，Master 用将块句柄和副本的位置返回客户端。 客户端使用文件名和块索引作为密钥缓存此信息，避免再与 Master 进行不必要的交互。缓存信息后，客户端向其中一个副本（通常是最接近的副本）发送请求，指定块句柄和该区块内的字节范围。 如果客户端需要再次读取相同的区块，在缓存的信息过期或文件重新打开前无需与 Master 交互。 实际上，客户端可以在同一个请求中请求多个块，而 Master 也同时返回多个块索引，这有助于减少后续客户端与 Master 的交互，而无额外开销。
...
​	GFS 的 Master 存储三种类型的元数据。第一种元数据是文件和块命名空间（**目录树**），用于跟踪系统中的文件和区块。第二种是从**文件到块的映射**，用于定位构成文件的块。所有元数据都保存在 Master 中，便于快速访问和检索信息。为保障元数据的持久性，系统会在 Master 上将变更落盘并在复制到远程的操作日志中。日志提供了对 Master 状态简单可靠的更新，在 Master 宕机时不用担心一致性问题。但是，Master 不会永久存储块位置信息。<u>在 Master 启动以及有新的块服务器加入集群时，向每个块服务器拉取块信息</u>。这种方法允许系统动态调整群集中的变化，例如添加或删除区块服务器。总体而言，Master 在管理 GFS 元数据方面起着至关重要的作用，其设计是性能、可靠性和可扩展性之间的平衡。
...
​	最初，作者试图将块位置信息永久保存在 Master，但在启动时以及之后定期向 ChunkServer 请求数据要简单得多。 此设计消除了在 ChunkServers 加入和离开集群、更改名称、失败、重启等时保持 Master 和块服务器同步的问题。在拥有数百台服务器的集群中，这些事件经常发生。
...
​	由于**操作日志**至关重要，因此必须对其进行可靠存储，并在变更持久化前，不应让客户端感知变更。如果在变更持久化前已对客户端可见，则即使块本身仍然存在，整个文件系统或最近的客户端操作也可能会丢失。为确保可靠性，将在多节点上复制操作日志，只有在本地和远程都将日志落盘后，才会响应客户端操作。Master 在刷新前将多个日志打包批处理，从而减少刷新和复制对整体系统吞吐量的影响。

​	Master 通过**重播操作日志**来恢复其文件系统状态。 为了最大限度地缩短启动时间，日志必须保持较小。 每当日志超过一定大小时，Master 都会检查其状态。 <u>检查点以类似 B 树的紧凑形式创建</u>，可以直接映射到内存中，无需额外解析即可用于命名空间查找。 这进一步加快了恢复速度并提高了可用性。
...
​	GFS 采用**松一致性模型**，以支持高度分布的应用，系统的设计易于使用和维护，在一致性和性能之间进行的权衡。 尽管其一致性模型比较宽松，但 GFS 仍然为应用程序提供数据准确性和时效性保障。GFS如何维持这些担保的细节留给了本文的其他部分。  
...
​	Client 可以在信息刷新前从旧的副本中读取数据，在缓存失效或再次打开文件时会更新副本元数据。 由于 GFS 中的大多数文件都是仅追加的，因此旧副本通常会返回过早的区块结尾，而不是过时的数据。当读者重试并联系主服务器时，它会立即获得当前的区块位置。GFS 通过主服务器和所有区块服务器之间的心跳来识别异常块服务器，并通过校验和检测数据损坏。发现问题后，数据会尽快从有效副本中恢复，只有在 GFS 做出反应之前（通常在几分钟之内）所有副本都丢失时，区块才会不可逆转地丢失。即使在这种情况下，区块只会变得不可用而不是损坏，应用程序会收到明显的错误而不是损坏的数据。
...
延期请求和授权随着 Master 和所有 chunkserver 间的 HeartBeat 传递。有时，Master 可能会尝试在租约到期之前撤销租约，例如当 Master 想要对正在重命名的文件禁用变更时。即使 Master 与 Primary 间断通信终端，它也可以在旧租约到期后安全地向另一个副本授予新的租约。
...
​	GFS的 Master 负责所有**命名空间的操作**。主节点还管理整个系统的**区块副本**，决定块**放置位置**、新**块创建**及**块备份**。 它还根据需要创建新的区块和副本，协调系统范围内的活动以保持区块的副本数，从而确保每个块都有足够的副本来容忍故障。 还可以平衡系统中所有区块服务器的负载，确保没有服务器因数据过多或请求过多而过载。 最后，Master 负责回收未使用的存储空间，为新数据腾出区块服务器上的空间。
...
​	Master 在操作之前，系统会根据所执行的操作获取一组锁，以确保操作可以安全地执行，不受其他操作的干扰。如对/d1/d2/.../dn/leaf的访问， 操作前系统先获取叶节点的路径中所有目录的读锁，包括 /d1、/d1/d2、... 和 /d1/d2/.../dn。再根据操作类型获取**叶节点的完整路径名上获取读锁或写锁** (/d1/d2/.../dn/leaf)。请务必注意，叶节点可能是文件或目录，具体取决于正在执行的操作。 
...
​	区块**副本放置策略**有两个主要目标：最大限度地提高数据的可靠性和可用性，以及最大限度地提高网络带宽利用率。仅仅将副本分布在计算机上不足以实现这些目标。这种方法只能防止磁盘或计算机故障，并充分利用每台计算机的网络带宽。为确保数据的可靠性和可用性，区块副本还必须分布在不同机架上。在单个机架损坏或断连时，区块的某些副本仍保持可用。跨机架存放区块副本使得块的操作（尤其是读取）可利用多个机架的总带宽。这意味着可以更快、更高效地处理读取请求。但是，写入流量必须流经多个机架，这是 GFS 做出的权衡。这种方法可确保即使面对硬件故障或网络中断，数据也能可靠地存储和使用。



## 创建、再复制和再平衡 creation、re-replication and rebalancing

​	GFS 创建区块副本有三个原因：**块创建**、**再复制**和**再平衡**。
​	创建区块时，Master 会根据几个因素选择将最初的空副本放置在何处。

1.   第一个因素是在**磁盘空间利用率**低于平均水平的区块服务器上放置新的副本。这样做是为了在一段时间内平衡区块服务器的磁盘利用率。
2.   第二个因素是限制每个区块服务器上的 **“最近” 创建的数量**。尽管创建本身很便宜，但它可以可靠地预测即将到来的大量写入流量，因为数据块是在写入需要时创建的。在 GFS 的一次追加多次读取负载模式下，数据块在完全写入后通常会变成只读状态。
3.   第三个因素是将区块的副本**分布在机架**上。这样做是为了在机架出现故障时提供容错能力。通过将副本分布在机架上，系统可确保单个机架故障不会导致区块的所有副本丢失。
...
​	GFS 群集通常在数百台计算机上有数千个磁盘，这意味着数据损坏或丢失的磁盘故障很常见。 GFS 中的每个区块服务器都使用校验和检测存储数据的损坏。 如果检测到数据损坏，GFS 可以使用其他区块副本从中恢复。但是，通过比较跨区块服务器的副本来检测损坏是不切实际的，因为由于 GFS 的原子性尾部追加语义，不同的副本可能都是合法的。 因此，每个区块服务器必须通过维护校验和来独立验证其自身副本的完整性。这样可以确保每个区块服务器都能检测到其自己的数据副本中可能发生的任何损坏。 

​	在 GFS 中，一个 chunk 被分为多个 64 KB 的 block，每个块都有相应的 32 位校验和。这些校验和保存在内存中，并与用户数据分开存储，与日志一起永久存储。

​	在收到 client 或 其他 ckunkserver 的读取请求时，chunkserver 会先验证与读取范围重叠的数据块的校验和，然后再将数据返回。 如果区块与记录的校验和不匹配，区块服务器会向请求者返回错误并向主服务器报告不匹配情况。收到错误后，请求者将从其他副本中读取数据，而 Master 将从其他正常副本中克隆一个块。 在新副本拷贝完成后，Master 会通知发送错误请求的区块服务器删除错误副本。 

​	尽管使用了校验和，但由于多种原因，读取性能并未受到显著影响。 由于大多数读取至少跨越几个区块，因此只需要读取和校验相对较少的额外数据即可进行验证。 GFS 客户端通过尝试对齐读取完整的校验和块，进一步减少了这种开销。 此外，区块服务器上的校验和比较无需任何 I/O 即可完成，校验和计算通常可以与 I/O 重叠。

​	GFS 对追加到区块末尾的写入操作的校验和计算过程进行了大量优化，因为此类写入在中占系统工作负载的绝大部分。 对于此类写入，系统会以增量方式更新最后一个部分校验和块的校验和，并为追加填充的块计算新的校验和。 这种方法可确保即使最后一个部分校验和块已经损坏同时系统未检测到，，但在下次块读取时由于新校验和与存储数据不匹配检测到损坏情况。 

​	但是，如果写入操作覆盖了区块的现有范围，则系统必须读取并验证被覆盖的范围中的第一个和最后一个块，然后执行写入，最后计算并记录新的校验和。 这是因为如果系统在部分覆盖第一个和最后一个区块之前未对其进行验证，则新的校验和可能会隐藏未被覆盖的区域中存在的损坏。 

​	在空闲期间，区块服务器可以扫描和验证非活动区块的内容，以检测很少读取的区块中的损坏情况。 检测到损坏后，Master 可以创建一个新的未损坏的副本并删除损坏的副本，以防止非活动但已损坏的区块副本欺骗主节点以为它有足够的有效区块副本。



## 诊断工具

​	GFS 会生成诊断日志，记录重大事件（如区块服务器的启动和关闭）以及所有 RPC 请求和响应。 这些日志被广泛用于问题隔离、调试和性能分析，而且成本很低。 如果没有日志，就很难理解机器之间暂时的、不可重复的交互，这可能会导致系统故障。系统可以自由删除诊断日志，而不会影响系统的正确性。但是，系统会尽量在空间允许的范围内保留这些日志。

​	RPC 日志包括通过网络发送的确切请求和响应，但读取或写入的文件数据除外。 通过匹配请求与响应进行以及整理不同计算机上的 RPC 记录，可以重建整个交互历史记录以诊断问题。 日志还可用作负载测试和性能分析的跟踪，这有助于识别瓶颈并提高系统性能。

​	日志记录对性能的影响微乎其微，因为这些日志是按顺序和异步写入的，这意味着它们不会影响系统的性能。 最新事件也保存在内存中，可供持续在线监控，这有助于实时识别问题。 这些诊断日志可以自由删除，而不会影响系统的正确性，但只要空间允许，它们就会被保存，以确保它们可供将来分析。



# 测试指标

作者在其中介绍了一些与Google文件系统（GFS）相关的微基准测试和真实测量结果。

## 微基准

​	集群由一个 Maser 、两个 Master 副本、16 个区块服务器和 16 个客户端组成。需要注意的是，设置此配置是为了便于测试，而典型的集群有数百个区块服务器和客户端。 集群中的所有计算机都配置了双 1.4 GHz PIII 处理器、2 GB 内存、两个 80 GB 5400 rpm 磁盘以及与 HP 2524 交换机的 100 Mbps 全双工以太网连接。 19 台 GFS 服务器计算机连接到一台交换机，16 台客户端连接到另一台交换机。 两台交换机通过 1 Gbps 链路连接，允许服务器和客户端之间进行高速通信。

### 读取

​	使用 N 个客户端从 320 GB 文件集中读取随机选择的 4 MB 区域。每个客户端重复此过程 256 次，最终每个客户端读取 1 GB 的数据。 本实验中使用的区块服务器只有 32 GB 的内存，这意味着 Linux 缓冲区缓存命中率预计最多为 10%。 实验结果接近冷缓存结果，这意味着数据未缓存在内存中，必须从磁盘读取。 

​	图 3 (a) 显示了 N 个客户端的总读取速率及其理论限制。当两台交换机之间的 1 Gbps 链路饱和时，理论限制的总值为 125 Mb/s，或者当其 100 Mbps 的网络接口饱和时，每台客户端 12.5 Mb/s（视情况而定）。实验中观察到的读取速率为 10 Mb/s，即每个客户端上限的 80%，而只有一个客户端在读取。 随着读取器数量的增加，16 个读取器的总读取速率达到了 94 Mb/s，约占125 Mb/s链接限制的75％，即每个客户端 6 Mb/s。 效率从 80% 下降到 75%，因为随着读取器数量的增加，多个读取器同时从同一个区块服务器读取数据的可能性也增加了。 

![image-20230830230527391](./images/gfs/image-20230830230527391.png)

### 写入

​	N 个客户端同时写入 N 个不同文件时。 每台客户机在一系列 1 MB 的写入操作中将 1 GB 的数据写入一个新文件。 总写入速率及其理论极限如图 3 (b) 所示。 理论上限稳定在 67 Mb/s，因为每个字节都需要写入 16 个区块服务器中的 3 个，每个区块服务器的输入连接为 12.5 Mb/s。

​	一台客户机的写入速率为 6.3 MB/s，约为理论限制的一半。 造成这种情况的主要原因是网络堆栈，它与用于将数据推送到区块副本的流水线方案交互效果不佳。 将数据从一个副本传播到另一个副本的延迟会降低总体写入速率。 

​	16 个客户机的总写入速率达到 35 MB/s（或每个客户端 2.2 MB/s），约为理论限制的一半。 随着客户端数量的增加，多个客户端同时写入同一个区块服务器的可能性越来越大。 16 位写入者比 16 位读者更有可能发生碰撞，因为每次写入都涉及三个不同的副本。 

​	写入速度比预期的要慢，但这并不是一个主要问题，因为它不会显著影响系统向大量客户端传送的聚合写入带宽。虽然单个客户端的写入速度可能会变慢，但整个系统仍然能够处理大量写入请求并以合理的速度传送这些请求。 

### 记录追加

​	图 3 (c)显示了 GFS 中记录追加操作的性能。 在此实验中，N 个客户端同时追加到一个文件中。 操作的性能受到存储文件最后一块的区块服务器的网络带宽的限制。 性能与客户端数量无关。 一台客户机的起始性能为 6.0 Mb/s，16 台客户机的性能下降到 4.8 MB/s。 性能下降主要是由于拥塞以及不同客户端看到的网络传输速率差异所致。

​	Google 中的应用程序倾向于同时生成多个文件，N 个客户端同时附加到 M 个共享文件中，其中 N 和 M 都为数十或数百个。 因此，本实验中的区块服务器网络拥塞在实践中并不是一个重大问题，因为客户端可以在写入一个文件方面取得进展，而另一个文件的区块服务器却处于繁忙状态。



## 真实集群

​	将讨论Google内部的两个具有代表性的集群。 集群 A 由一百多名工程师用于研发，而集群 B 主要用于生产数据处理。 在集群 A 中，典型的任务由人启动，运行时间长达几个小时。在此期间，它会读取几 MB 到几 TB 的数据，在转换或分析后将结果写回集群。 相比之下，集群 B 中的任务持续时间要长得多，并且只需偶尔的人为干预即可持续生成和处理 multi-TB 数据集。 在这两个集群中，一个 “任务” 包含多台计算机上的多进程对多文件的同时读取和写入。

​	表 2 提供了两个 GFS 群集的特征，包括区块服务器数量、可用和已用磁盘空间、文件数量、死文件数、区块数以及区块服务器和主服务器上的元数据。 集群 A 有 342 个区块服务器、72 TB 的可用磁盘空间、55 TB 的已用磁盘空间、73.5 万个文件、2.2 万个死文件、99.2 万个区块、13 GB 的元数据位于区块服务器，主服务器上有 48 MB 的元数据。 集群 B 有 227 个区块服务器、180 TB 的可用磁盘空间、155 TB 的已用磁盘空间、73.7 万个文件、23.7 万个死文件、155 万个区块、21 GB 的元数据位于区块服务器，主服务器上有 60 MB 的元数据。 

![image-20230830231833804](./images/gfs/image-20230830231833804.png)

### 存储

​	两个集群都有数百个区块服务器，负责存储和管理数据块。 集群支持 TB 的磁盘存储量，使用率较高但未完全满，仍有一些存储空间可用于存储其他数据。 “已用空间” 指标包括所有区块副本。 集群中的几乎所有文件都会维护三副本。集群分别存储 18 TB 和 52 TB 的文件数据。

​	这两个群集的文件数量相似，但集群 B 的失效文件比例更大。失效文件是指已删除或被新版本替换，但其存储空间尚未被回收的文件。 集群 B 的区块也比其他群集多，因为它的文件往往较大。

### 元数据

​	区块服务器主要存储 64 KB 用户数据块的校验和，需要存储数十 GB 的元数据。 保存在区块服务器的另一个元数据是区块版本号。 

​	存储在 Master 上的元数据要小得多，只有几十 MB，即平均每个文件大约 100 字节。 Master 的元数据大小很小，这与这样的假设一致，即 Master 的内存大小不会成为系统瓶颈。 文件的大部分元数据都是以前缀压缩形式存储的文件名。 其他元数据包括文件所有权和权限、从文件到区块的映射以及每个区块的当前版本。 对于每个区块，系统存储当前副本位置和用于实现写时复制的引用计数。 

​	区块服务器和主服务器都只有 50 到 100 MB 的元数据，这使得恢复速度更快。 在服务器能够响应查询前，恢复只需几秒钟即可从磁盘加载到内存。 但是，Master 通常徐亚 30 到 60 秒从所有区块服务器获取区块位置信息。

### 读写速率

​	表 3 描述了不同时间段的读取和写入速率数据。 测量前两个集群都已运行大约一周。刚启动时，平均写入速率低于 30 MB/s，这表明系统的性能未达到最佳状态。 测量时，其中一个集群 (B) 正在经历写入活动爆发，生成了大约 100 MB/s 的数据。 如此高的写入速率导致网络负载为 300 MB/s，因为写入会传播到三个副本。 

​	读取负荷高于写入，这是与假设吻合，两个集群均处在高负荷状态。特别是，集群 A 在前一周一直保持 580 MB/s 的读取速率，这是很高的数据读取速率。 集群 A 的网络配置最高可支持 750 Mb/s，这意味着它可以高效地使用其资源来处理工作负载。 另一方面，集群 B 可以支持 1300 MB/s 的峰值读取速率，但其应用程序仅使用 380 MB/s，这意味着它没有充分利用其资源。

![image-20230830233323183](./images/gfs/image-20230830233323183.png)

### Master Load

​	表 3 显示，发送到 Maser 的操作速率约为每秒 200 到 500 次操作。Master 可以很容易地跟上这个速度，因此不是这些工作负载的瓶颈。 但是，在 GFS 的早期版本中，Maseter 偶尔会成为某些工作负载的瓶颈。因为它花了大部分时间按顺序扫描包含数十万个文件的大型目录，寻找特定的文件。 为了解决此问题，更改了 Master 的数据结构，允许通过命名空间进行高效的二进制搜索。 Master 现在可以轻松支持每秒数千次文件访问。如有必要，可以通过在命名空间数据结构前放置名称查询缓存来进一步优化系统。 

### 恢复时长

​	当区块服务器出现故障时，某些区块副本数少于所需的复制级别，必须对其进行克隆。 恢复所有副本不足的区块所需的时间取决于可用于克隆的资源量。 在一项实验中，集群 B 中的单个区块服务器出现故障，该群集有大约 15,000 个区块包含 600 GB 的数据。 为了限制对正在运行的应用程序的影响并为调度决策提供余地，默认参数限制了并发克隆的数量和每次克隆操作的最大速度。 在本实验中，默认参数允许 91 个并发克隆，其中允许每个克隆操作最多消耗 6.25 Mb/s（50 Mbps）。 所有区块均在 23.2 分钟内恢复，这意味着有效复制速率为 440 MB/s。 在另一项实验中，两台区块服务器分别有大约 16,000 个区块和 660 GB 的数据，出现故障，将 266 个区块减少到只有一个副本。 这 266 个区块以更高的优先级克隆，并在 2 分钟内全部恢复到至少 2 倍的副本。 这使集群处于可以容忍另一个区块服务器故障而不会丢失数据的状态。



## 工作负载拆解

​	本节介绍两个 GFS 集群的工作负载，其规模与 6.2 节中讨论的集群相当，但并不相同。集群 X 用于研发目的，而集群 Y 用于生产数据处理。

### 设计方法论及限制

​	结果基于客户对 GFS 的请求，但不包括服务器间请求或内部后台活动，例如转发写入或重新平衡，这可能会影响文件系统的总体工作负载。为了收集有关 I/O 操作的统计信息，作者使用了从 GFS 服务器记录的实际 RPC 请求中经过启发式重建的信息。例如，如果 GFS 客户端代码将读取操作分成多个 RPC 以提高并行度，则作者会从这些 RPC 中推断出原始读取量。 -作者承认他们的访问模式是高度风格化的，这意味着他们的工作负载可能无法代表其他应用程序或文件系统。但是，他们希望将数据中的任何错误降至最低。 -作者指出，应用程序的显式日志记录本来可以提供稍微准确一点的数据，但是从逻辑上讲，不可能重新编译和重启成千上万个正在运行的客户端，而且从尽可能多的机器上收集结果会很麻烦。 -作者提醒读者不要从工作量中过于概括，因为Google对GFS及其应用程序的控制意味着它们彼此之间进行了高度调整。这种相互影响也可能存在于一般应用程序和文件系统之间，但这种影响可能更为明显。

### 	块服务器负载

​	表 4 显示了 GFS 中按大小划分的操作分布。系统中的读取大小呈双峰分布，这意味着分布中有两个峰值。 小于 64 KB 的大小读取是由经常在大文件中寻找小块数据的客户端生成的。 另一方面，大小超过 512 KB 的大读取是由对整个文件执行长顺序读取的客户端生成的。 在集群 Y 中，大量读取操作根本不返回任何数据。这是因为这些文件被用作生产者-使用者队列，生产者将数据附加到文件中，而使用者则读取文件末尾。有时，当消费者超过生产者时，不会返回任何数据。 集群 X 不太频繁地显示这种情况，因为它通常用于短期的数据分析任务，而不是耗时长的分布式应用程序。 系统中的写入大小也呈现出双峰分布。写入量超过 256 KB 的大量写入通常是由于写入器内部存在大量缓冲所致。 缓冲较少的数据、更频繁地进行检查点或同步，或者干脆生成较少数据的写入会占较小的写入（大小低于 64 KB）。 就记录追加而言，集群 Y 看到的大型记录追加的百分比比集群 X 高得多。这是因为使用集群 Y 的生产系统更积极地针对 GFS 进行了调整。 

![image-20230830234923130](./images/gfs/image-20230830234923130.png)

​	表5显示了在各种规模的操作中传输的数据总量。对于所有类型的操作，较大的操作（大小超过 256 KB）通常占传输的大部分字节。 -但是，由于随机寻道的工作负载，大小低于 64 KB 的小量读取确实会传输一小部分但重要的读取数据。

![image-20230830235141400](./images/gfs/image-20230830235141400.png)
