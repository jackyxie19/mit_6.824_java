...
​	如前所述，块会被放置在不同机架上的多个区块服务器上，以确保容错能力和数据可用性。 用户可以灵活地为不同目录指定不同的复制级别，默认复制级别为三。 Master 会维护块的副本数量，以防区块服务器离线或校验到损坏的副本。 尽管复制是有效的，但 GFS 仍在探索其他形式的跨服务器冗余，例如奇偶校验码或擦除码，以满足他们不断增长的只读存储需求。 与复制相比，奇偶校验码或擦除码可以提供更有效的存储利用率和更快的恢复时间。 虽然这些更复杂的冗余方案实现起来可能具有挑战性，但却易于管理以大流量追加写和读取为主的场景。
